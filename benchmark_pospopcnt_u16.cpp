...
int pospopcount_u16_wrapper(std::string name,
    STORM_pospopcnt_u16_func f, 
    int iterations,
    uint32_t range,
    uint32_t n_values,
    uint32_t n_bitmaps, 
...
asm   volatile("RDTSCP\n\t"
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        f((uint16_t*)mem, n_values, histogram);
        for (int j=0; j < 32; j++)
            total += histogram[j];
    }
...
int benchmark(int n_repetitions, bool use_perf = false) {
    ...
    for (int i = 0; i < ranges.size(); ++i) {
        ...
        puts("");
        pospopcount_u16_wrapper("scalar",               &STORM_pospopcnt_u16_scalar_naive, reps[i], ranges[i], ranges[i], ranges[i], unit_intsec);
        pospopcount_u16_wrapper("umul128",              &STORM_pospopcnt_u16_scalar_umul128_unroll2, reps[i], ranges[i], ranges[i], ranges[i], unit_intsec);
        pospopcount_u16_wrapper("SSE SAD",              &STORM_pospopcnt_u16_sse_sad, reps[i], ranges[i], ranges[i], ranges[i], unit_intsec);
        pospopcount_u16_wrapper("SSE blend unroll8",    &STORM_pospopcnt_u16_sse_blend_popcnt_unroll8, reps[i], ranges[i], ranges[i], ranges[i], unit_intsec);
        pospopcount_u16_wrapper("SSE Harley-Seal",      &STORM_pospopcnt_u16_sse_harvey_seal, reps[i], ranges[i], ranges[i], ranges[i], unit_intsec);
    }
